...
class SeedlingImagerGUI(QWidget):
    ...
    def show_experiment_snapshot(self, plate_idx: int, frame):
        ...
        EXTENDED (Sept 2026): experiment_runner.py now also emits this
        signal with frame=None for any plate immediately following a
        skipped (not captured) plate — a real save_image() capture is the
        only thing that reliably flushes the camera stream, so a snapshot
        grabbed right after a skip can't be trusted (confirmed on real
        hardware: it can show a frame belonging to an earlier plate
        entirely). frame=None is handled explicitly below by clearing the
        preview instead of just returning, so a stale picture from an
        earlier plate can't linger on screen next to a status message that
        ...
        if frame is None:
            ...
            self.camera_label.setText("No preview\n(plate not captured this cycle)")
            self.status_label.setText(f"Plate #{plate_idx}: preview unavailable")
            return
